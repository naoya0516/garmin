...
対象読者は実装担当者（`garmin-coder`）。本書のシグネチャ・処理手順は実装時の初期方針であり、`garminconnect`の実際のインストール済みバージョンのdocstring等で齟齬が判明した場合は、動作を優先して調整してよい（その場合も本書を追随して更新することが望ましい）。
...
> **05_API設計.mdへ正式に反映済み**: F-03（[02_機能一覧](02_機能一覧.md)・[03_画面設計](03_画面設計.md)で承認済み）に対応する`GET /api/activities/summary`エンドポイントは、[05_API設計](05_API設計.md)のエンドポイント一覧表および専用セクションに正式に転記済み（パス名・スキーマとも本書の記載どおりで確定）。以下の`summary.py`はその実装詳細を示すものであり、05の内容と齟齬はない。詳細は「[基本設計書との整合性メモ](#基本設計書との整合性メモ)」参照。
...
2. **サマリー取得のAPIエンドポイントは[05_API設計](05_API設計.md)へ正式に反映済み**: 本項目はもともと、F-03に対応するエンドポイントが`05_API設計.md`のエンドポイント一覧表に記載されていないという記載漏れを指摘していたものである。その後、01〜03で承認済みの仕様を技術文書に反映する作業として（新規のUX判断・機能追加ではないため再レビューは不要と判断のうえ）、`05_API設計.md`のエンドポイント一覧表および`GET /api/activities/summary`セクションに本書のパス名・スキーマ（`ActivitySummaryOut`/`RunningSummary`）がそのまま正式に転記された。したがって本書の`summary.py`・`GET /api/activities/summary`の記載は提案・暫定案ではなく、05と整合した確定仕様として扱ってよい。
